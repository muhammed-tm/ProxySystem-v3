            ...
            for (String word : words) {
                for (String arg : event.getMessage().getContentRaw().split(" ")) {
                    if (arg.equalsIgnoreCase(word)) {
                        ...
                        builder.setTitle("Chatfilter");
                        builder.setColor(Color.RED);
                        builder.addField("Achtung der User " + event.getAuthor().getAsTag() + " hat ein Unerlaubtes Wort genutzt."
                                , "Nachricht: " + event.getMessage().getContentRaw()
                                        + " (" + arg + ")", false);
            ...
            for (String domain : ads) {
                for (String arg : event.getMessage().getContentRaw().split(" ")) {
                    if (arg.contains(domain)
                            && !arg.contains("discordapp")
                            && !arg.contains("tenor")
                            && !arg.contains("youtube")
                            && !arg.contains("hypetime")) {
                        EmbedBuilder builder = new EmbedBuilder();
                        builder.setTitle("Chatfilter");
                        builder.setColor(Color.RED);
                        builder.addField("Achtung der User " + event.getAuthor().getAsTag() + " hat eine Domain gesendet."
                                , "Nachricht: " + event.getMessage().getContentRaw(), false);
                        Objects.requireNonNull(event.getGuild().getTextChannelById("724649430514401360"))
                                .sendMessage(builder.build()).queue();
                        if(event.getMessage() != null) {
                            event.getMessage().delete().queue();
